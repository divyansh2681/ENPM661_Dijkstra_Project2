...
def obstacles(any_list):
    # circle
    for x in range(240, 360, 1):
        for y in range(120, 250, 1):
...
black = (0,0,0)
white = (0,255,255)
surf = pygame.surfarray.make_surface(canvas_copy_visited)
...
for path in closed_list_coord:
    ...
    x = path[1]
    y = path[0]
    new_canvas_copy_backtrack[(249-y,x)]=[255,0,0] # setting every backtracked pixel to white
...
for path in backtrack_coord:
    x = path[1]
    y = path[0]
    new_canvas_copy_backtrack[(249-y,x)]=[0,255,0] # setting every backtracked pixel to green
# showing the final backtracked path
new_backtracked = cv2.resize(new_canvas_copy_backtrack,(800,500))
